...
class Slide {
  ...
  _updateImagesSize() {
    ...
    if (image) {
      ...
      if (!image.dataset.largestUsedSize || width > image.dataset.largestUsedSize) {
  ...
  calculateZoomToPanOffset(axis, point, prevZoomLevel) {
    ...
    const imageSize =
      this[axis === 'x' ? 'width' : 'height'] * (prevZoomLevel || this.currZoomLevel);
...
class MainScroll {
  ...
  resize(resizeSlides) {
    ...
    this.slideWidth = Math.round(pswp.viewportSize.x + pswp.viewportSize.x * pswp.options.spacing);
...
class ScrollWheel {
  ...
  _onWheel(e) {
    ...
    if (e.ctrlKey || this.pswp.options.wheelToZoom) {
      // zoom
      if (currSlide.isZoomable()) {
        const destZoomLevel = currSlide.currZoomLevel - currSlide.zoomLevels.fit * 0.04 * deltaY;
...
function getThumbBounds(index, itemData, instance) {
  ...
  const thumbnail = element.matches(thumbSelector) ? element : element.querySelector(thumbSelector);
...
class PhotoSwipeBase extends Eventable {
  ...
  _domElementToItemData(element) {
    ...
    // NOTE: This line is changed from dataset.pswpSrcset. See
    // a7bc33597a8942f810c2b348926c8e6a0f21d9b1
    itemData.srcset = linkEl.dataset.pswpResponsive;
